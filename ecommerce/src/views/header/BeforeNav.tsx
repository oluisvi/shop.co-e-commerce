...
export default function BeforeNav() {
    const [isVisible, setIsVisible] = useState(true);

    if (!isVisible) return null; // Se estiver fechado, não renderiza nada

    return (
        <div>
            <article className="before-nav">
                <div className="up-cookie">
                    <p>
                        Sign up and get 20% off your first order.{" "}
                        <a href="#" className="underline-cookie">Sign Up Now</a>
                    </p>
                    <button className="close-icon" onClick={() => setIsVisible(false)}>
                        <svg
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"

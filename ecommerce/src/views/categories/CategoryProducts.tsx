/* eslint-disable @next/next/no-img-element */
function CategoryProducts() {
    return(
        
        <>

        <section className="category-products">
            <div className="container-category-products">
                <article className="category-page-header">
                    <article className="category-title">
                        <h2>Casual</h2>
                    </article>

                    <article className="category-pages">
                        <article className="category-page-count">
                            <p>Showing 1-10 of 100 Products</p>
                        </article>
                        <article className="category-page-filter">
                            <p>Sort By: <span>Most Popular</span></p>
                            <img src="/assets/down-sight.svg" alt="" />
                        </article>
                    </article>
                </article>
                <article className="products-category-pages">
                <article className="product-1">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-1.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Polo with Contrast Trims</p>
                </div>
                <div className="product-rating">
                  <img src="/assets/full-star.svg" alt="star" />
                  <img src="/assets/full-star.svg" alt="star" />
                  <img src="/assets/full-star.svg" alt="star" />
                  <img src="/assets/full-star.svg" alt="star" />
                  <span className="rating-score">4/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$212</p>
                  </div>
                  <div className="product-discount">$242</div>
                  <div className="discount-tag">-20%</div>
                </div>
              </article>
              <article className="product-2">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-2.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Gradient Graphic T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/half-star.svg" alt="star" />
                  <span className="rating-score">3.5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$145</p>
                  </div>
                </div>
                </article>
                <article className="product-3">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-3.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Polo with Tipping Deta</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/half-star.svg" alt="star" />
                  <span className="rating-score">4.5/5</span>
                </div>
                <div className="product-price">
                  <p>$180</p>
                </div>
                </article>
                <article className="product-4">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-4.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Black Striped T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                  <span className="rating-score">5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$120</p>
                  </div>
                  <div className="product-discount">$150</div>
                  <div className="discount-tag">-30%</div>
                </div>
                </article>
                <article className="product-4">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-4.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Black Striped T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                  <span className="rating-score">5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$120</p>
                  </div>
                  <div className="product-discount">$150</div>
                  <div className="discount-tag">-30%</div>
                </div>
              </article>
              <article className="product-2">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-2.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Gradient Graphic T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/half-star.svg" alt="star" />
                  <span className="rating-score">3.5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$145</p>
                  </div>
                </div>
              </article>
              <article className="product-3">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-3.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Polo with Tipping Deta</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/half-star.svg" alt="star" />
                  <span className="rating-score">4.5/5</span>
                </div>
                <div className="product-price">
                  <p>$180</p>
                </div>
              </article>
              <article className="product-4">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-4.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Black Striped T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                  <span className="rating-score">5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$120</p>
                  </div>
                  <div className="product-discount">$150</div>
                  <div className="discount-tag">-30%</div>
                </div>
              </article>
              <article className="product-4">
                <div className="clothes-card">
                  <img
                    src="/assets/you-might-also-like-4.png"
                    alt="t-shirt"
                    className="clothes-img"
                  />
                </div>
                <div className="product-name">
                  <p>Black Striped T-shirt</p>
                </div>
                <div className="product-rating">
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                <img src="/assets/full-star.svg" alt="star" />
                  <span className="rating-score">5/5</span>
                </div>
                <div className="product-sale">
                  <div className="product-price">
                    <p>$120</p>
                  </div>
                  <div className="product-discount">$150</div>
                  <div className="discount-tag">-30%</div>
                </div>
              </article>
            </article>  
            <article className="category-page-footer">
                <article className="preview-page-btn">
                    <img src="/assets/left-arrow.svg" alt="" />
                    <p>Previous</p>
                </article>
                <article className="actual-page-selection">
                <div className="first-pages">
                    <div className="page-1">
                    <p>1</p>
                    </div>
                    <div className="page-2">
                    <p>2</p>
                    </div>
                    <div className="page-3">
                    <p>3</p>
                    </div>
                </div>
                <div className="middle-separation">
                    <p>...</p>
                </div>
                <div className="last-pages">
                    <div className="page-8">
                    <p>8</p>
                    </div>
                    <div className="page-9">
                    <p>9</p>
                    </div>
                    <div className="page-10">
                    <p>10</p>
                    </div>
                    
                </div>
                </article>
                <article className="next-page-btn">
                <p>Next</p>
                <img src="/assets/right-arrow.svg" alt="" />
                </article>
            </article>
            </div>            
        </section>

        </>

    );
}

export default CategoryProducts;